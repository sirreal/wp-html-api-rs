...
const UNICODE_REPLACEMENT_CHAR: &[u8] = &[b'\xEF', b'\xBF', b'\xBD'];
...
pub fn decode_html(ctx: HtmlContext, input: &[u8], offset: usize, len: usize) -> &[u8] {
    todo!()
}
...
fn html5_character_reference_is_ambiguous(end_of_match: &[u8]) -> bool {
    static AMBIGUOUS_FOLLOWERS: [bool; 255] = [
    ...
    return b';' != end_of_match[0] && AMBIGUOUS_FOLLOWERS[end_of_match[1] as usize];
}
...
fn html5_code_point_to_utf8_bytes(code_point: u32) -> Box<[u8]> {
    ...
    if code_point <= 0x7F {
        decoded[0] = code_point as u8;
        return decoded[0..1].into();
    }

    if code_point <= 0x7FF {
        decoded[0] = ((code_point >> 6) | 0xC0) as u8;
        decoded[1] = ((code_point & 0x3F) | 0x80) as u8;
        return decoded[0..2].into();
    }

    if code_point <= 0xFFFF {
        decoded[0] = ((code_point >> 12) | 0xE0) as u8;
        decoded[1] = (((code_point >> 6) & 0x3F) | 0x80) as u8;
        decoded[2] = ((code_point & 0x3F) | 0x80) as u8;
        return decoded[0..3].into();
    }

    decoded[0] = ((code_point >> 18) | 0xF0) as u8;
    decoded[1] = (((code_point >> 12) & 0x3F) | 0x80) as u8;
    decoded[2] = (((code_point >> 6) & 0x3F) | 0x80) as u8;
    decoded[3] = ((code_point & 0x3F) | 0x80) as u8;
    return decoded.into();
}
