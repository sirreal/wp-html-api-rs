...
impl HtmlProcessor {
    ...
    fn step_in_body(&mut self) -> Result<bool, HtmlError> {
        let token_name = self.get_token_name().unwrap_or_default();
        let token_type = self.get_token_type();
        let is_closer = self.is_tag_closer();
        
        let op = Op::from_token(token_type, is_closer, &token_name);
        ...
        match op {
             ...
             */
            Op::None(Token::Text) => {
                if self.text_node_classification == TextNodeClassification::NullSequence {
                 ...
                 */
                if self.text_node_classification == TextNodeClassification::Generic {
             ...
             * > A comment token
             */
            Op::None(Token::Comment | Token::FunkyComment | Token::PresumptuousTag) => {
                self.insert_html_element(self.state.current_token.as_ref().unwrap().clone());
                Ok(true)
            },
             ...
             */
            Op::Open(TagName::Html) => {
                if !self.state.stack_of_open_elements.contains("TEMPLATE") {
                    /*
             ...
             */
            Op::Open(TagName::Base | TagName::Basefont | TagName::Bgsound | TagName::Link | 
                    TagName::Meta | TagName::Noframes | TagName::Script | TagName::Style | 
                    TagName::Template | TagName::Title) |
            Op::Close(TagName::Template) => {
                self.step_in_head()
            },
             ...
             */
            Op::Open(TagName::Body) => {
                if !self.state.stack_of_open_elements.contains("TEMPLATE") {
                    /*
                     * > Otherwise, for each attribute on the token, check to see if the attribute
                     * > is already present on the second element of the stack of open elements. If
                     * > it is not, add the attribute and its corresponding value to that element.
                     *
                     * This parser does not currently support this behavior: ignore the token.
                     */
             ...
             */
            Op::Open(TagName::Frameset) => {
                if !self.state.frameset_ok {
                    // Parse error: ignore the token.
                    return self.step();
                }

                // @todo Implement frameset support.
                self.bail("Frameset elements are not yet supported.")
            },

            /*
             * > An end tag whose tag name is "body"
             */
            Op::Close(TagName::Body) => {
                if !self.state.stack_of_open_elements.has_element_in_scope("BODY") {
                    // Parse error: ignore the token.
                    return self.step();
                }
                 ...
                 */
                self.state.insertion_mode = InsertionMode::AfterBody;
                Ok(true)
            },
            ...
            Op::Close(TagName::Html) => {
                 ...
                 */
                self.state.insertion_mode = InsertionMode::AfterBody;
                return self.step(NodeToProcess::ReprocessCurrentNode);
            },
             ...
             */
            Op::Open(TagName::Address | TagName::Article | TagName::Aside | TagName::Blockquote |
                    TagName::Center | TagName::Details | TagName::Dialog | TagName::Dir |
                    TagName::Div | TagName::Dl | TagName::Fieldset | TagName::Figcaption |
                    TagName::Figure | TagName::Footer | TagName::Header | TagName::Hgroup |
                    TagName::Main | TagName::Menu | TagName::Nav | TagName::Ol | TagName::P |
                    TagName::Section | TagName::Summary | TagName::Ul) => {
                if self.state.stack_of_open_elements.has_element_in_button_scope("P") {
             ...
             */
            Op::Open(TagName::H1 | TagName::H2 | TagName::H3 | TagName::H4 | TagName::H5 | TagName::H6) => {
                if self.state.stack_of_open_elements.has_element_in_button_scope("P") {
                    self.close_a_p_element()?;
                }
             ...
             */
            Op::Open(TagName::Pre | TagName::Listing) => {
                if self.state.stack_of_open_elements.has_element_in_button_scope("P") {
                    self.close_a_p_element()?;
                }
            ...
            Op::Open(TagName::Form) => {
                if self.state.stack_of_open_elements.contains("TEMPLATE") {
                    ...
                    if self.state.form_element.is_some() {
                        // Parse error: ignore the token.
                        return self.step();
                    }
                    ...
                    let form_element = self.insert_html_element(self.state.current_token.as_ref().unwrap().clone());
                    self.state.form_element = Some(form_element);
                    Ok(true)
                }
            },
            ...
            Op::Open(TagName::Table) => {
                ...
                self.insert_html_element(self.state.current_token.as_ref().unwrap().clone());
                self.state.frameset_ok = false;
                self.state.insertion_mode = InsertionMode::InTable;
                Ok(true)
            },
             ...
             */
            Op::Close(TagName::Address | TagName::Article | TagName::Aside | TagName::Blockquote |
                     TagName::Button | TagName::Center | TagName::Details | TagName::Dialog |
                     TagName::Dir | TagName::Div | TagName::Dl | TagName::Fieldset |
                     TagName::Figcaption | TagName::Figure | TagName::Footer | TagName::Header |
                     TagName::Hgroup | TagName::Listing | TagName::Main | TagName::Menu |
                     TagName::Nav | TagName::Ol | TagName::Pre | TagName::Section |
                     TagName::Summary | TagName::Ul) => {
                if !self.state.stack_of_open_elements.has_element_in_scope(&token_name) {
                    // Parse error: ignore the token.
                    return self.step();
                }

                self.generate_implied_end_tags(None)?;
                if self.state.stack_of_open_elements.current_node().map_or(false, |n| n.node_name.to_string() != token_name) {
                    // Parse error.
                }

                self.state.stack_of_open_elements.pop_until(&token_name);
                Ok(true)
            },
             ...
             */
            Op::Close(TagName::Form) => {
                if self.state.stack_of_open_elements.contains("TEMPLATE") {
                    if !self.state.stack_of_open_elements.has_element_in_scope("FORM") {
                        // Parse error: ignore the token.
                        return self.step();
                    }

                    self.generate_implied_end_tags(None)?;
                    if self.state.stack_of_open_elements.current_node().map_or(false, |n| n.node_name != TagName::Form) {
                        // Parse error.
                    }

                    self.state.stack_of_open_elements.pop_until("FORM");
                    Ok(true)
                } else {
                    let form_element = self.state.form_element.take();
                    ...
                    self.state.stack_of_open_elements.remove_from_stack("FORM");
                    Ok(true)
                }
            },
            ...
            Op::Close(TagName::P) => {
                if !self.state.stack_of_open_elements.has_element_in_button_scope("P") {
                    // Parse error.
                    self.insert_html_element(HTMLToken::new_tag("P", false));
                }
             ...
             */
            Op::Close(TagName::Li) => {
                if !self.state.stack_of_open_elements.has_element_in_list_item_scope("LI") {
                    // Parse error: ignore the token.
                    return self.step();
                }

                self.generate_implied_end_tags(Some(TagName::Li))?;
                if self.state.stack_of_open_elements.current_node().map_or(false, |n| n.node_name != TagName::Li) {
                    // Parse error.
                }

                self.state.stack_of_open_elements.pop_until("LI");
                Ok(true)
            },
            ...
            _ => {
                ...
                match op {
                    Op::Close(tag_name) => {
                         ...
                         * >    error; return (do not process the token).
                         * > 4. Set node to the previous entry in the stack of open elements.
                         * > 5. Return to the step labeled loop.
                         */
                        if self.state.stack_of_open_elements.has_element_in_scope(&tag_name) {
                            self.generate_implied_end_tags(Some(tag_name.clone()))?;
                            if self.state.stack_of_open_elements.current_node().map_or(false, |n| n.node_name != tag_name) {
                                // Parse error.
                            }
...
/// Represents an HTML operation during parsing.
/// Used for making parsing decisions based on token type and name.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum Op {
    /// Represents an opening tag with its name
    Open(TagName),
    /// Represents a closing tag with its name
    Close(TagName),
    /// Represents a non-tag token
    None(Token),
}

impl Op {
    /// Creates a new Op based on token type and closer status
    pub fn from_token(token_type: Option<TokenType>, is_closer: bool, token_name: &str) -> Self {
        match token_type {
            Some(TokenType::Tag) if is_closer => Op::Close(TagName::from(token_name)),
            Some(TokenType::Tag) => Op::Open(TagName::from(token_name)),
            _ => Op::None(Token::from(token_name)),
        }
    }
}
